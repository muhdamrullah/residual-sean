...
# ##################### Get IDs from Roof-Data Kaggle dataset #########################
...
def load_data():
    ...
    # Unpickle and process for unlabelled images only
    d = unpickle('./unlabelled_batch')
    xs.append(d['data'])
    ys.append(d['label'])
...
def main(n=5, num_epochs=82, model=None):
    ...
    X_test = data['X_test']
    Y_test = data['Y_test']
    X_unlabelled = data['X_unlabelled']
    Y_unlabelled = data['Y_unlabelled']
...
#      batch_predictions = batch_predictions[:,1]>batch_predictions[:,0]
    ...
    np.savetxt("submission-4.csv", submission_array,  fmt='%d', delimiter=',', newline='\n', header='Id,label', comments = '')
